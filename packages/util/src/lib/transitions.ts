...
export const transitions = () => {
    return {
        ...
        'ease-linear': {
            utility: { 'transition-timing-function': 'linear' },
            meta: { group: 'transitionTimingFunction', order: 1 },
        },
        'ease-in': {
            utility: { 'transition-timing-function': 'cubic-bezier(0.4, 0, 1, 1)' },
            meta: { group: 'transitionTimingFunction', order: 2 },
        },
        'ease-out': {
            utility: { 'transition-timing-function': 'cubic-bezier(0, 0, 0.2, 1)' },
            meta: { group: 'transitionTimingFunction', order: 3 },
        },
        'ease-in-out': {
            utility: { 'transition-timing-function': 'cubic-bezier(0.4, 0, 0.2, 1)' },
            meta: { group: 'transitionTimingFunction', order: 4 },
        },
...
function gen(type: string, title: string) {
    ...
    for (const index of ipt) {
        rs[`${type}-${index}`] = JSON.parse(
            `{"utility": {"transition-${type}": "${index}ms"}, "meta": {"group": "${title}", "order": ${index}}}`,
        );
    }
    return rs;
}
